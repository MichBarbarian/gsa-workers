...
|---|---|---|---|---|---|---|
| 1 | [`wallet_nonce_balance_daily`](../workers/wallet_nonce_balance_daily/README.md) | Claim | 6/12/18 (matrix a/b; no 00:00 — BD night window) | `wallets` + daily flags | `wallet_apply_daily_snapshot` | `wallet_daily_metrics` (flat); rollup `wallet_rollup_daily_metrics` → `wallet_transactions` |
...
| 9 | [`wallet_activity_flows`](../workers/wallet_activity_flows/README.md) | Claim (`wallet_transactions`, matrix 4) | 1/15 00:00 + every 4h drain (00–24 UTC) | `is_valid_activity_flows` + due clock + not `Dormant_*` + valid agent | `wallet_activity_transfers_insert` | Staging `wallets.wallet_activity_transfers` (INSERT-only) |
...
**Live (schema must be applied first).** Matrix 4 cells by provider group. Cron days **1 and 15** 00:00 UTC plus drain every **4h** (full day 00–24) until the claim queue is empty (`exit 0`). INSERT-only into `wallets.wallet_activity_transfers`. Does not compute Walcert metrics or DELETE staging.
